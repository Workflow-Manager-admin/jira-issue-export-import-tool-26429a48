...
async def refresh_session(current_session: dict = Depends(get_current_session)):
    """
    Refresh current session expiration time
    """
    try:
        success = auth_service.refresh_session(current_session['session_token'])
        if not success:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
...
async def logout(current_session: dict = Depends(get_current_session)):
    ...
    """
    try:
        success = auth_service.invalidate_session(current_session['session_token'])
        if not success:
            raise HTTPException(
...
async def get_projects(current_session: dict = Depends(get_current_session)):
    """
    ...
    try:
        ...
        project_responses = []
        for i, jira_project in enumerate(jira_projects):
            project_response = ProjectResponse(
                id=i + 1,  # Simple incrementing ID since no database
...
async def get_project_details(
    ...
    try:
        ...
        jira_project = jira_service.get_project_details(project_key)
        ...
        return ProjectResponse(
            id=1,  # Simple ID since no database
            jira_project_key=jira_project.get('key'),
            jira_project_id=jira_project.get('id'),
            name=jira_project.get('name', ''),
            description=jira_project.get('description', ''),
...
async def get_project_issue_types(
    ...
    try:
        ...
        for i, jira_issue_type in enumerate(jira_issue_types):
            ...
            issue_type_responses.append(issue_type_response)
